  ...
  return (
    <>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <Badge variant="outline" className={cn("text-xs font-semibold", paymentConfig.className)}>
          {paymentConfig.label}
        </Badge>
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 md:py-10">
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
            ...
            {orders.map((order, i) => {
              ...
              return (
                <button
                  key={order.id}
                  type="button"
                  onClick={(e) => { (e.currentTarget as HTMLElement).blur(); setSelectedOrder(order) }}
        ...
        {/* Order Detail Panel — Desktop sidebar */}
        {selectedOrder && (
          <aside
            key={selectedOrder.id}
            className="hidden h-fit rounded-xl border border-border bg-card p-5 lg:block lg:w-96"
          >
            <OrderDetailContent
              order={selectedOrder}
              items={selectedItems}
      ...
      {/* Order Detail — Mobile drawer */}
      <Drawer open={!!selectedOrder && isMobile} onOpenChange={(open) => { if (!open) setSelectedOrder(null) }}>
        <DrawerContent aria-describedby={undefined}>
          <DrawerTitle className="sr-only">
            {selectedOrder ? `Order ${selectedOrder.id.slice(0, 8).toUpperCase()}` : "Order Details"}
          </DrawerTitle>
          {selectedOrder && (
